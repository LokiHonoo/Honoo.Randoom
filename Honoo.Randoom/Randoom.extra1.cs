...
namespace Honoo
{
    public partial class Randoom
    {
        ...
        public string NextString(int count, char token)
        {
            ...
            switch (token)
            {
                case 'd': case '1': positions = GetPositions(_digitalLess.Length, count); source = _digitalLess; break;
                case 'D': case '0': positions = GetPositions(_digital.Length, count); source = _digital; break;
                case 'a': positions = GetPositions(_alphabetLess.Length, count); source = _alphabetLess; break;
                case 'A': positions = GetPositions(_alphabet.Length, count); source = _alphabet; break;
                case 'm': positions = GetPositions(_mixtureLess.Length, count); source = _mixtureLess; break;
                case 'M': positions = GetPositions(_mixture.Length, count); source = _mixture; break;
                case 'h': positions = GetPositions(_hex.Length, count); source = _hex; break;
        ...
        public string NextString(int count, string customSource)
        {
            ...
            List<int> positions = GetPositions(customSource.Length, count);
            var result = new StringBuilder();
            foreach (int position in positions)
            {
                result.Append(source[position]);
